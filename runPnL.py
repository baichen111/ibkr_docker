...
from accountInfo import acc,saveDir  # load account info
...
ib = IB()
ib.connect('127.0.0.1', 7496, clientId=6)  #gateway port 4001; TWS UI 7496
...
for port in portItems:
    if port.contract.secType == "OPT":
        con_id[port.contract.conId] = f"port.contract.symbol_Option{opt_cnt}_{port.contract.right}_{port.contract.strike}"
        opt_cnt += 1
    else:
        con_id[port.contract.conId] =port.contract.symbol
...
def save_df(df: pd.DataFrame, path: str = saveDir):
    """
    ...
    print('Saved csv file',path+file_name)
    print("Saving down csv file to ",path + "DailyPnL.csv")
    df.to_csv(path + "DailyPnL.csv")
    print("Saved csv file to ",path + "DailyPnL.csv")
